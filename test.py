...
class Wiiboard:
    ...
    def weight(self, weight):
        self.blink(5)
        rweight = round(weight, 1)        
        who = "stranger"
        if rweight >= 8.0 and weight <= 12.0:
            who = "Rango"
        elif rweight < 8.0:
            who = "Pip"
        elif rweight > 100.0 and rweight < 160.0:
            who = "Katherine"
        elif rweight > 180.0:
            who = "Mark"
        print "{0} weight is {1} lbs.".format(who, weight)
        # self.post_to_website(who, rweight)
    
    def post_to_website(self, who, weight):
        params = urllib.urlencode({"name": who, "weight": weight})
        userAndPass = b64encode(os.environ["userpass"]).decode("ascii")
        headers = {"Content-Type": "application/x-www-form-urlencoded", "Accept": "text/plain", 'Authorization' : 'Basic %s' %  userAndPass }
        conn = httplib.HTTPSConnection(os.environ["url"])
        conn.request("POST", "/weight", params, headers)
        response = conn.getresponse()

def main():
    processor = EventProcessor()
    board = Wiiboard(processor)
    processor.addListener(board)
    ...
    board.wait(200)
    # print "Connected.  Trying to calabrate..."
    # board.calibrate()
    # print "Complete"
